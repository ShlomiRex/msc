...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \coordinate (MIDDLE_BEGIN) at ($(Z_TEXT) + (-0.25, -0.5)$);
        \coordinate (MIDDLE_END) at ($(Z_TEXT) + (2.25, 1)$);
        \coordinate (MU_BEGIN) at ($(MIDDLE_BEGIN) + (0.25, 0.75)$);
        \coordinate (MU_END) at ($(MU_BEGIN) + (0.75, 0.5)$);
        \coordinate (SIGMA_BEGIN) at ($(MIDDLE_BEGIN) + (0.75, -0.5)$);
        \coordinate (SIGMA_END) at ($(SIGMA_BEGIN) + (0.75, 0.5)$);

        % Big rect, Mu, Sigma
        % \draw[draw=black] (MIDDLE_BEGIN) rectangle (MIDDLE_END);
        \draw (MU_BEGIN) rectangle (MU_END) node[pos=.5] {$\mu$};
        \draw (SIGMA_BEGIN) rectangle (SIGMA_END) node[pos=.5] {$\sigma$};
